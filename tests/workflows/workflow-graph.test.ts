/**
 * Purpose:
 * Unit tests for WorkflowGraph (LangGraph StateGraph).
 */

import { describe, it, expect, vi } from 'vitest';
import { buildWorkflowGraph, createInitialState } from '@/workflows/workflow-graph';
...
describe('WorkflowGraph', () => {
  ...
  describe('createInitialState', () => {
    it('should create state with DRAFT status', () => {
      const state = createInitialState('wf_001', 'prod_001', 'Portable Blender');

      expect(state.currentState).toBe(WorkflowState.DRAFT);
      expect(state.context.workflowId).toBe('wf_001');
      expect(state.context.productIdea).toBe('Portable Blender');
    });
  });

  describe('buildWorkflowGraph', () => {
    it('should build a compilable graph', () => {
      const graph = buildWorkflowGraph(mockAI);
    ...
    it('should execute graph from initial state', async () => {
      const graph = buildWorkflowGraph(mockAI);
      const initialState = createInitialState('wf_002', 'prod_002', 'Smart Watch');
